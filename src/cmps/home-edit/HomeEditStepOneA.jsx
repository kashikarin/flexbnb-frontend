...
export function HomeEditStepOneA() {
  ...
  return (
    <section
      className="home-edit-step-one-a-container"
      ...
      <article className="home-edit-step-one-a-buttons-container">
        {gHomeTypes.map((homeType, i) => {
          return (
            <button
              key={`${homeType}${i}`}
              className={`${type === gHomeTypes[i] ? 'selected' : ''}`}
              onClick={() =>
                updatePotentialHome({ ...potentialHome, type: gHomeTypes[i] })
              }
            >
              <ReactSVG
                src={`/svgs/home-types/home-type${i + 1}.svg`}
                className="home-type-icon"
              />
